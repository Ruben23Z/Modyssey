# SQL schema for Mod Odyssey (Steam integration removed)
...
VALUES ('guest'),('user'),('sympathizer'),('admin');
...
-- Mod versioned changelog
CREATE TABLE mod_version (
    IDModVersion INT NOT NULL AUTO_INCREMENT,
    mod_id INT NOT NULL,
    version VARCHAR(20) NOT NULL,
    changelog TEXT NOT NULL,
    file_path VARCHAR(255) NOT NULL,
    created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    PRIMARY KEY (IDModVersion),
    CONSTRAINT fk_modversion_mod FOREIGN KEY (mod_id) REFERENCES `mod` (IDMod) ON DELETE CASCADE
);

-- Mod ↔ Category many‑to‑many
...
-- User subscriptions (games or categories)
CREATE TABLE user_subscription (
    ...
    PRIMARY KEY (id),
    UNIQUE KEY uq_user_sub_game (user_id, game_id),
    UNIQUE KEY uq_user_sub_category (user_id, category_id),
    CONSTRAINT fk_us_user FOREIGN KEY (user_id) REFERENCES user(IDUser) ON DELETE CASCADE,
    CONSTRAINT fk_us_game FOREIGN KEY (game_id) REFERENCES game(IDGame) ON DELETE CASCADE,
    CONSTRAINT fk_us_category FOREIGN KEY (category_id) REFERENCES category(IDCategory) ON DELETE CASCADE
